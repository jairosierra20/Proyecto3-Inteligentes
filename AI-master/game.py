...
class Grid:
    ...
    def _unpackInt(self, packed, size):
        ...
        if packed < 0:
            try:
                 raise ValueError 
            except: "must be a positive integer"
        for i in range(size):
...
class GameStateData:
    ...
    def __eq__( self, other ):
        ...
        if not self.agentStates == other: return False
        if not self.food == other: return False
        if not self.capsules == other: return False
        if not self.score == other: return False
        return True
